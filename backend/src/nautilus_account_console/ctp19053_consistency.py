...
DEFAULT_SOURCE_PACKAGE = ROOT / "output" / "account_capability" / "ctp-paper-19053" / "source-package.json"
DEFAULT_BLOCKER_PATH = ROOT / "docs" / "acceptance" / "2026-06-15-ctp19053-real-readback-blocker.json"
...
class ConsistencyResult:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": "ctp19053_real_readback_result.v1",
...
def evaluate_ctp19053_source_package(source_package: Path = DEFAULT_SOURCE_PACKAGE) -> ConsistencyResult:
    ...
    if command["enabled"] is not False or command["mode"] != "disabled":
        raise Ctp19053ConsistencyError("CTP 19053 command capability must remain disabled")
    return ConsistencyResult(
        verdict="passed",
        account_id=ACCOUNT_ID,
        blocker_id=None,
...
def write_blocker_if_needed(
    ...
    result = evaluate_ctp19053_source_package(source_package)
    if result.verdict == "blocked":
        blocker_path.parent.mkdir(parents=True, exist_ok=True)
        blocker_payload = {
            **result.to_dict(),
            "owner": "nautilus_ctp_adapter",
            "template_ref": "contracts/source_artifacts/templates/ctp_paper_19053_source_package.template.json",
            "next_action": "Produce pinned read-only CTP Paper 19053 source package at output/account_capability/ctp-paper-19053/source-package.json from the template and source-owner read-only query output",
            "reason": "CTP Paper 19053 real source package is not available inside the current worktree.",
        }
...
def _validate_source_package(payload: dict[str, Any], path: Path) -> None:
    ...
    if missing:
        raise Ctp19053ConsistencyError(f"{path}: missing required keys {missing}")
    if payload["account_id"] != ACCOUNT_ID:
        raise Ctp19053ConsistencyError(f"{path}: account_id must be {ACCOUNT_ID}")
    if payload["display_alias"] != "19053":
        raise Ctp19053ConsistencyError(f"{path}: display_alias must be 19053")
    if payload["source_owner"] != "nautilus_ctp_adapter":
        raise Ctp19053ConsistencyError(f"{path}: source_owner must be nautilus_ctp_adapter")
    if payload["source_kind"] != "ctp_trader_api":
        raise Ctp19053ConsistencyError(f"{path}: source_kind must be ctp_trader_api")
    if payload["source_mode"] not in {"paper_observation", "delayed_snapshot"}:
        raise Ctp19053ConsistencyError(f"{path}: source_mode must be paper_observation or delayed_snapshot")
    if payload["account_domain"] != "paper":
        raise Ctp19053ConsistencyError(f"{path}: account_domain must be paper")
    if payload["observation_mode"] != "snapshot":
        raise Ctp19053ConsistencyError(f"{path}: observation_mode must be snapshot")
    if payload["event_stream"] != "not_implemented":
        raise Ctp19053ConsistencyError(f"{path}: event_stream must be not_implemented")
    ...
    if not str(payload["source_checksum"]).startswith("sha256:"):
        raise Ctp19053ConsistencyError(f"{path}: source_checksum must be sha256")
    if _contains_sensitive_key(payload):
        raise Ctp19053ConsistencyError(f"{path}: sensitive credential-like field detected")
