...
| P024 partial-fill owner repair evidence ingest audit | `python scripts\validate_p024_partial_fill_owner_repair_evidence_ingest_audit.py`; `npx playwright test tests/e2e/p024-partial-fill-owner-repair-ingest-audit.spec.ts --project=desktop` then `python scripts\validate_p024_partial_fill_owner_repair_ingest_audit_browser_evidence.py` | `P024_PARTIAL_FILL_OWNER_REPAIR_EVIDENCE_INGEST_AUDIT_OK`; `P024_PARTIAL_FILL_OWNER_REPAIR_INGEST_AUDIT_BROWSER_EVIDENCE_OK` | Owner repair commit/checksum/validator refs are recorded and visible in Web UI while runtime retry and real partial-fill claims remain false |
| P024 partial-fill post-repair runtime retry approval packet | `python scripts\validate_p024_partial_fill_post_repair_runtime_retry_approval_packet.py`; `npx playwright test tests/e2e/p024-partial-fill-post-repair-runtime-retry-packet.spec.ts --project=desktop` then `python scripts\validate_p024_partial_fill_post_repair_runtime_retry_packet_browser_evidence.py` | `P024_PARTIAL_FILL_POST_REPAIR_RUNTIME_RETRY_APPROVAL_PACKET_OK`; `P024_PARTIAL_FILL_POST_REPAIR_RUNTIME_RETRY_PACKET_BROWSER_EVIDENCE_OK` | At most one guarded exposure-reduction paper attempt is authorized after owner repair evidence and packet updates; execution and partial-fill claims remain false until runtime artifacts exist |
| P024 partial-fill post-repair runtime attempt audit | `python scripts\validate_p024_partial_fill_post_repair_runtime_attempt_audit.py`; `npx playwright test tests/e2e/p024-partial-fill-post-repair-runtime-attempt.spec.ts --project=desktop` then `python scripts\validate_p024_partial_fill_post_repair_runtime_attempt_browser_evidence.py` | `P024_PARTIAL_FILL_POST_REPAIR_RUNTIME_ATTEMPT_AUDIT_OK`; `P024_PARTIAL_FILL_POST_REPAIR_RUNTIME_ATTEMPT_BROWSER_EVIDENCE_OK` | The one authorized owner attempt full-filled 1 lot and reduced exposure; it is explicitly non-qualifying for partial-fill/cancel acceptance and retry is consumed |
...
| A34 | positive | Owner repair evidence is ingested with commit/checksum/validator refs | artifact validator + Playwright + API route audit + browser evidence JSON | owner repair evidence is missing, secret-bearing, validator-failing, or claims runtime/partial-fill completion | phase4zd_owner_repair_evidence_ingested |
| A35 | positive | Post-repair runtime retry is packet-authorized for at most one guarded attempt | artifact validator + Playwright + API route audit + browser evidence JSON | packet allows more than one attempt, omits exposure-reduction guard, or claims runtime already ran | phase4ze_post_repair_runtime_retry_approval_packet_ready |
| A36 | blocker | Post-repair runtime attempt outcome is classified without overstating partial-fill acceptance | owner artifact refs/checksums + Playwright + API route audit + browser evidence JSON | full fill is counted as partial-fill/cancel acceptance, retry remains authorized, or raw secrets are recorded | phase4zf_post_repair_runtime_attempt_full_fill_blocker_recorded |
