...
The partial-fill cancel display evidence records `ui_command_control_verdict=historical_fixture_governed_cancel_intent_display_only` to make clear that this browser fixture is archive-scoped governed UI evidence only and does not promote the cancel intent path to runtime truth.
...
| P024 partial-fill runtime approval packet UI projection | archive-only historical browser evidence retained at `docs/acceptance/browser-evidence/p024-account-console-paper-command-controls/partial-fill-runtime-approval-packet-ui.json`; active runtime API/UI surface retired | `P024_PARTIAL_FILL_RUNTIME_APPROVAL_PACKET_BROWSER_EVIDENCE_OK` | Historical UI evidence remains reviewable without keeping the live runtime projection mounted |
...
| P024 partial-fill runtime handoff bundle UI projection | archive-only historical browser evidence retained at `docs/acceptance/browser-evidence/p024-account-console-paper-command-controls/partial-fill-runtime-handoff-bundle-ui.json`; active runtime API/UI surface retired | `P024_PARTIAL_FILL_RUNTIME_HANDOFF_BUNDLE_BROWSER_EVIDENCE_OK` | Historical UI evidence remains reviewable without keeping the live runtime projection mounted |

The live route and UI panel remain retired; archive-only historical browser evidence remains machine-checkable without remounting the pre-repair runtime surface.
The live owner-repair execution-lane routes and UI panels remain retired; archive-only historical browser evidence remains machine-checkable without remounting the completed repair lane.
...
| P024 partial-fill owner repair approval packet UI projection | archive-only historical browser evidence retained at `docs/acceptance/browser-evidence/p024-account-console-paper-command-controls/partial-fill-owner-repair-approval-packet-ui.json`; active runtime API/UI surface retired | `P024_PARTIAL_FILL_OWNER_REPAIR_APPROVAL_PACKET_BROWSER_EVIDENCE_OK` | Historical UI evidence remains reviewable without keeping the live owner-repair planning projection mounted |
| P024 partial-fill remaining acceptance state UI projection | archive-only historical browser evidence retained at `docs/acceptance/browser-evidence/p024-account-console-paper-command-controls/partial-fill-remaining-acceptance-state-ui.json`; active runtime API/UI surface retired | `P024_PARTIAL_FILL_REMAINING_ACCEPTANCE_STATE_BROWSER_EVIDENCE_OK` | Historical UI evidence remains reviewable without keeping the live owner-repair planning projection mounted |
...
| P024 partial-fill owner repair ingest gate UI projection | archive-only historical browser evidence retained at `docs/acceptance/browser-evidence/p024-account-console-paper-command-controls/partial-fill-owner-repair-ingest-gate-ui.json`; active runtime API/UI surface retired | `P024_PARTIAL_FILL_OWNER_REPAIR_INGEST_GATE_BROWSER_EVIDENCE_OK` | Historical UI evidence remains reviewable without keeping the live owner-repair execution-lane projection mounted |
...
| P024 partial-fill owner repair preflight UI projection | archive-only historical browser evidence retained at `docs/acceptance/browser-evidence/p024-account-console-paper-command-controls/partial-fill-owner-repair-preflight-ui.json`; active runtime API/UI surface retired | `P024_PARTIAL_FILL_OWNER_REPAIR_PREFLIGHT_BROWSER_EVIDENCE_OK` | Historical UI evidence remains reviewable without keeping the live owner-repair execution-lane projection mounted |
...
| P024 partial-fill owner repair patch preview UI projection | archive-only historical browser evidence retained at `docs/acceptance/browser-evidence/p024-account-console-paper-command-controls/partial-fill-owner-repair-patch-preview-ui.json`; active runtime API/UI surface retired | `P024_PARTIAL_FILL_OWNER_REPAIR_PATCH_PREVIEW_BROWSER_EVIDENCE_OK` | Historical UI evidence remains reviewable without keeping the live owner-repair execution-lane projection mounted |
...
| P024 partial-fill owner repair execution handoff UI projection | archive-only historical browser evidence retained at `docs/acceptance/browser-evidence/p024-account-console-paper-command-controls/partial-fill-owner-repair-execution-handoff-ui.json`; active runtime API/UI surface retired | `P024_PARTIAL_FILL_OWNER_REPAIR_EXECUTION_HANDOFF_BROWSER_EVIDENCE_OK` | Historical UI evidence remains reviewable without keeping the live owner-repair execution-lane projection mounted |
| P024 partial-fill owner repair evidence ingest audit | `python scripts\validate_p024_partial_fill_owner_repair_evidence_ingest_audit.py` | `P024_PARTIAL_FILL_OWNER_REPAIR_EVIDENCE_INGEST_AUDIT_OK` | Archive-only artifact gate: owner repair commit/checksum/validator refs remain machine-checked after the runtime API/UI surface is retired |
| P024 partial-fill post-repair runtime retry approval packet | `python scripts\validate_p024_partial_fill_post_repair_runtime_retry_approval_packet.py` | `P024_PARTIAL_FILL_POST_REPAIR_RUNTIME_RETRY_APPROVAL_PACKET_OK` | Archive-only artifact gate: the single guarded retry authorization remains machine-checked after the runtime API/UI surface is retired |
| P024 partial-fill post-repair runtime attempt audit | `python scripts\validate_p024_partial_fill_post_repair_runtime_attempt_audit.py` | `P024_PARTIAL_FILL_POST_REPAIR_RUNTIME_ATTEMPT_AUDIT_OK` | Archive-only artifact gate: the full-fill, non-qualifying retry outcome remains machine-checked without keeping a live runtime projection |
...
| A17 | positive | Partial-fill runtime approval packet historical UI evidence remains archived after owner-repair lane supersedes the live surface | archive-only browser evidence + artifact validator | archive evidence disappears, or live surface is still required for active acceptance | phase4l_partial_fill_runtime_approval_packet_ui_projection_passed |
| A18 | positive | Partial-fill runtime handoff bundle historical UI evidence remains archived after owner-repair lane supersedes the live surface | archive-only browser evidence + artifact validator | archive evidence disappears, or live surface is still required for active acceptance | phase4m_partial_fill_runtime_handoff_bundle_ui_projection_passed |
...
| A34 | positive | Owner repair evidence is ingested with commit/checksum/validator refs | artifact validator (archive-only historical evidence) | owner repair evidence is missing, secret-bearing, validator-failing, or claims runtime/partial-fill completion | phase4zd_owner_repair_evidence_ingested |
| A35 | positive | Post-repair runtime retry is packet-authorized for at most one guarded attempt | artifact validator (archive-only historical evidence) | packet allows more than one attempt, omits exposure-reduction guard, or claims runtime already ran | phase4ze_post_repair_runtime_retry_approval_packet_ready |
| A36 | blocker | Post-repair runtime attempt outcome is classified without overstating partial-fill acceptance | owner artifact refs/checksums (archive-only historical evidence) | full fill is counted as partial-fill/cancel acceptance, retry remains authorized, or raw secrets are recorded | phase4zf_post_repair_runtime_attempt_full_fill_blocker_recorded |
...
Accepted evidence: `python scripts\validate_p024_partial_fill_runtime_feasibility_audit.py` returns `P024_PARTIAL_FILL_RUNTIME_FEASIBILITY_AUDIT_OK` with `typed_blocker_owner_partial_fill_runtime_evidence_missing`. This is a typed blocker audit, not a real partial-fill pass.
...
P024 browser evidence is now proposal-scoped: `python scripts\validate_p024_partial_fill_cancel_browser_evidence.py` returns `P024_PARTIAL_FILL_CANCEL_BROWSER_EVIDENCE_OK`. This is display-contract evidence only and explicitly preserves `historical_fixture_governed_blocked_until_owner_evidence`; it does not claim real partial-fill runtime or broker truth.
...
The API treats `account_id=acct.ctp.paper.19053` and `mode=paper_armed` as scope checks only, not authorization. Submit, cancel and runtime-run-request routes must fail closed with `command_capability_not_mounted` unless the selected account capability bundle declares `command.enabled=true`, `command.mode=paper_armed`, the requested action, `authority_ref=owner-repo://nautilus_ctp_adapter`, and a checksum.

The submit/cancel routes are capability-gated contract gates only. They do not send broker commands. The runtime handoff routes prepare typed owner-runtime run requests only after the same capability gate passes; they do not invoke owner scripts, send gateway commands or create broker orders. The runtime closeout route is read-only projection of existing owner-backed artifacts and must not return positive runtime claims from repo-local `output/` unless owner-runtime evidence is present.
...
2. A `paper_armed` UI projection without owner capability authority still shows no submit/cancel controls.
3. Submit and cancel are not emitted while `command_capability_not_mounted` is in force.
4. Browser evidence records `paper_armed_controls_visible=false`, `gateway_send_attempted=false`, `broker_order_created=false` and `gateway_ack_is_final_state=false`.
5. The cancel request is not constructed until command capability authority is mounted.
