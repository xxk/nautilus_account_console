...
| Phase 4l partial-fill runtime approval packet UI projection | Historical browser evidence is retained as archive-only after the active runtime API/UI surface is retired in favor of the owner-repair lane | `python scripts\validate_p024_partial_fill_runtime_approval_packet_browser_evidence.py` |
| Phase 4m partial-fill runtime handoff bundle UI projection | Historical browser evidence is retained as archive-only after the active runtime API/UI surface is retired in favor of the owner-repair lane | `python scripts\validate_p024_partial_fill_runtime_handoff_bundle_browser_evidence.py` |
...
| Phase 4v owner repair preflight source audit | Current owner source checksums prove the CLOSEYESTERDAY offset 4 repair evidence is still missing, so a scripts-only approval must not trigger a blind retry | `python scripts\validate_p024_partial_fill_owner_repair_preflight_source_audit.py` |
| Phase 4w owner repair preflight UI projection | Web UI renders the read-only owner source checksum audit and blind-retry rejection without owner write or runtime invocation claims | `python scripts\validate_p024_partial_fill_owner_repair_preflight_browser_evidence.py` |
| Phase 4x owner repair patch preview | Exact owner patch shape for CLOSEYESTERDAY offset 4 semantics and focused tests is frozen while owner write and runtime retry remain false | `python scripts\validate_p024_partial_fill_owner_repair_patch_preview.py` |
| Phase 4y owner repair patch preview UI projection | Web UI renders the patch preview baseline files, patch steps, validators and no-write/no-retry flags | `python scripts\validate_p024_partial_fill_owner_repair_patch_preview_browser_evidence.py` |
...
| Phase 4zd owner repair evidence ingest audit | Owner repair commit/checksum/validator refs are ingested without raw secrets; historical UI evidence is retained as archive-only and the live runtime API/UI projection is retired | `python scripts\validate_p024_partial_fill_owner_repair_evidence_ingest_audit.py` |
| Phase 4ze post-repair runtime retry approval packet | One guarded exposure-reduction paper attempt is authorized after owner validators and account-console packet updates; the historical UI projection is archive-only and the live runtime API/UI projection is retired | `python scripts\validate_p024_partial_fill_post_repair_runtime_retry_approval_packet.py` |
| Phase 4zf post-repair runtime attempt audit | The authorized owner attempt produced a real full fill of 1 lot and position reduction, not a partial-fill/cancel lifecycle; retry is consumed and the live runtime API/UI projection is retired | `python scripts\validate_p024_partial_fill_post_repair_runtime_attempt_audit.py` |
| Command-plane retirement guard | Canonical durable command truth is declared by `/api/commands/accounts/{account_id}/projection`, which points back to mirror ownership and marks legacy `/api/commands/*` read surfaces as retirement-bound read-only projections | `python scripts\validate_p024_paper_command_api.py` |
| Command-read retirement slicing | Canonical projection classifies legacy read surfaces into blocker projections that remain separate and retired archive-only panel-bound projections, and maps each retired surface to the exact frontend panel dependency and historical evidence owner | `python scripts\validate_p024_paper_command_api.py`; `/api/commands/accounts/{account_id}/projection` |
| First retirement batch | The first grouped retirement batch is complete for the consumed post-repair closeout lane: ingest-audit, retry-packet and attempt-audit routes plus their three UI panels are retired as archive-only evidence | `python scripts\validate_p024_paper_command_api.py`; `/api/commands/accounts/{account_id}/projection` |
| Second retirement batch | The second grouped retirement batch is complete for the superseded pre-repair attempt-planning lane: partial-fill runtime approval packet and handoff bundle routes plus their two UI panels are retired as archive-only evidence | `python scripts\validate_p024_paper_command_api.py`; `/api/commands/accounts/{account_id}/projection` |
| Third retirement batch | The third grouped retirement batch is complete for the superseded owner-repair planning lane: repair approval packet, remaining-acceptance current state and implementation-plan routes plus their three UI panels are retired as archive-only evidence | `python scripts\validate_p024_paper_command_api.py`; `/api/commands/accounts/{account_id}/projection` |
| Fourth retirement batch | The fourth grouped retirement batch is complete for the completed owner-repair execution lane: ingest gate, preflight source audit, patch preview and execution handoff routes plus their four UI panels are retired as archive-only evidence | `python scripts\validate_p024_paper_command_api.py`; `/api/commands/accounts/{account_id}/projection` |
...
| `docs/acceptance/p024-account-console-paper-command-controls/partial-fill-owner-repair-preflight-source-audit.json` | read-only owner source checksum audit proving repair is still required before a runtime retry |
| `docs/acceptance/browser-evidence/p024-account-console-paper-command-controls/partial-fill-owner-repair-preflight-ui.json` | Web UI projection evidence for the owner repair preflight source audit |
| `docs/acceptance/p024-account-console-paper-command-controls/partial-fill-owner-repair-patch-preview.json` | owner repair patch preview freezing the intended CLOSEYESTERDAY offset 4 source/test edits before owner write approval |
| `docs/acceptance/browser-evidence/p024-account-console-paper-command-controls/partial-fill-owner-repair-patch-preview-ui.json` | Web UI projection evidence for the owner repair patch preview |
| `docs/acceptance/p024-account-console-paper-command-controls/partial-fill-owner-repair-execution-handoff-bundle.json` | owner repair execution handoff bundle for post-approval patch, validators, evidence ingest and retry packet preparation |
| `docs/acceptance/browser-evidence/p024-account-console-paper-command-controls/partial-fill-owner-repair-execution-handoff-ui.json` | Web UI projection evidence for the owner repair execution handoff bundle |
...
| Real partial-fill runtime feasibility audit | archive_only | `partial-fill-runtime-feasibility-audit.json` records that the current one-order approval is consumed, no trade fill was observed, no deterministic partial-fill generator/artifact exists, and both non-UI and Web UI real partial-fill acceptance remain blocked behind the typed blocker verdict `typed_blocker_owner_partial_fill_runtime_evidence_missing` | phase4h_real_partial_fill_runtime_feasibility_blocked |
...
| Partial-fill runtime approval packet UI projection | archive_only | Historical browser evidence is retained, but the live route/panel is retired in the second retirement batch because owner-repair governance is now the canonical next step | phase4l_partial_fill_runtime_approval_packet_ui_projection_passed |
| Partial-fill runtime handoff bundle UI projection | archive_only | Historical browser evidence is retained, but the live route/panel is retired in the second retirement batch because owner-repair governance is now the canonical next step | phase4m_partial_fill_runtime_handoff_bundle_ui_projection_passed |
...
| Owner repair ingest gate UI projection | archive_only | Historical browser evidence is retained, but the live route/panel is retired in the fourth retirement batch because the completed execution lane is now governed as archive-only evidence | phase4u_owner_repair_ingest_gate_ui_projection_passed |
| Owner repair preflight source audit | archive_only | `partial-fill-owner-repair-preflight-source-audit.json` records current owner source checksums and rejects blind script retry until CLOSEYESTERDAY offset 4 repair evidence exists | phase4v_owner_repair_preflight_source_audited |
| Owner repair preflight UI projection | archive_only | Historical browser evidence is retained, but the live route/panel is retired in the fourth retirement batch because the completed execution lane is now governed as archive-only evidence | phase4w_owner_repair_preflight_ui_projection_passed |
| Owner repair patch preview | archive_only | `partial-fill-owner-repair-patch-preview.json` freezes the intended owner code/test patch shape while owner patch applied, validators, runtime retry and full acceptance remain false | phase4x_owner_repair_patch_preview_ready |
| Owner repair patch preview UI projection | archive_only | Historical browser evidence is retained, but the live route/panel is retired in the fourth retirement batch because the completed execution lane is now governed as archive-only evidence | phase4y_owner_repair_patch_preview_ui_projection_passed |
...
| Owner repair execution handoff UI projection | archive_only | Historical browser evidence is retained, but the live route/panel is retired in the fourth retirement batch because the completed execution lane is now governed as archive-only evidence | phase4za_owner_repair_execution_handoff_ui_projection_passed |
...
| Owner repair evidence ingest audit | archive_only | `partial-fill-owner-repair-evidence-ingest-audit.json` records owner commit `01db0f84203db09874fdec07ec8d03fb47ca4ea7`, source/test checksums and validator results while real partial-fill remains false; live route/panel retired in first retirement batch | phase4zd_owner_repair_evidence_ingested |
| Post-repair runtime retry approval packet | archive_only | `partial-fill-post-repair-runtime-retry-approval-packet.json` authorizes at most one guarded exposure-reduction paper attempt after owner repair evidence and packet updates; live route/panel retired in first retirement batch | phase4ze_post_repair_runtime_retry_approval_packet_ready |
| Post-repair runtime attempt audit | archive_only | `partial-fill-post-repair-runtime-attempt-audit.json` records the real owner paper order as full-filled, exposure-reducing, non-qualifying for partial-fill acceptance and retry-consumed; live route/panel retired in first retirement batch | phase4zf_post_repair_runtime_attempt_full_fill_blocker_recorded |
