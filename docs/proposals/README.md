...
| [P024 Account Console Paper Command Controls](./p024-account-console-paper-command-controls/README.md) | phase3a_runtime_closeout_and_phase3b_display_passed | ADR-0007 successor proposal for guarded Web/API paper submit/cancel controls on `acct.ctp.paper.19053`, including owner-backed runtime closeout projection and P024-scoped partial-fill then cancel Web UI display correctness; frontend controls are gated by `paper_armed` and call Phase 1 API, while browser-triggered broker gateway send and `live_armed` remain disabled. |
